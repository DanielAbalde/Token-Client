...
abstract contract TokenAbstraction is ERC165
{
    ...
    function isApproved(Token calldata token, address account, address operator) onlyStandard(token) external view virtual returns (bool){ return _isApproved(token, account, operator); }
    function isApprovedSet(TokenSet calldata tokenSet, address account, address operator) onlyStandardSet(tokenSet) external view virtual returns (bool){ return _isApprovedSet(tokenSet, account, operator); }
    function transfer(Token calldata token, address from, address to) onlyStandard(token) external virtual returns (bool){ return _transfer(token, from, to); }
    function transferSet(TokenSet calldata tokenSet, address from, address to) onlyStandardSet(tokenSet) external virtual returns (bool){ return _transferSet(tokenSet, from, to); }
